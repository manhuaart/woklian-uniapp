...
<!--      <div
      ...
      <view class="address" v-if="status.type == 0"  @click="addressTap">
		<view class="address-little">
	        ...
	        <view class="address-info">
	           ...
	           <view class="userAddress">地址:<text>{{orderInfo.userAddress}}</text></view>				
			</view>
			<view class="xiugai">修改 <span class="iconfont icon-jiantou"></span>   </view>
	    </view>	  
	  </view>
      <view class="address" v-else>
		<view class="address-little" v-if="orderInfo._status._type > -1">
			<view class="img-address"><image src="../../../static/images/weizhi.png"></image></view>
	        <view class="address-info">
			   <view class="name">
	              {{ orderInfo.realName }}
		...
		<view class="wrapper-tou" v-if="orderInfo._status._type < 0">
				<view class="item acea-row row-between">
					<view>补贴券退回</view>
					<view class="conter">
					  {{orderInfo.sumDiscount}}
					</view>
				</view>
				<view class="item acea-row row-between">
					<view>退回微信</view>
    ...
    <view class="wrapper" v-if="orderInfo._status._type >-1">
      <view class="item acea-row row-between">
        <view>订单编号：</view>
        <view class="conter ">
          {{ orderInfo.orderId }}
      ...
      <view class="item acea-row row-between">
        <view>补贴劵抵扣：</view>
        <view class="conter">-{{orderInfo.sumDiscount}}</view>
      </view>	  
<!--      <view class="item acea-row row-between">
        <view>支付方式：</view>
      ...
      <view class="item acea-row row-between">
        <view>订单备注：</view>
        <view class="conter" v-if="orderInfo.mark">{{ orderInfo.mark }}</view>
		<view class="conter" v-else>无</view>
      </view>
	  ...
	  <view class="actualPay acea-row row-right">
	      实付款： <text class="money font-color-red">￥{{ orderInfo.payPrice }}</text>
	  </view>
    </view>
...
<!--    <view class="wrapper" v-if="refundOrder">
    ...
    <!-- 退款订单详情 -->
   <view class="wrapper" v-if="orderInfo._status._type < 0">
      <view class="item acea-row row-between">
        <view>退款原因:</view>
      ...
      <view class="item acea-row row-between">
        <view>退款金额:</view>   
        <view class="conter font-color-red"><text>￥</text><text>{{ orderInfo.payPrice}}</text></view>
      </view>
      <view class="item acea-row row-between">
        <view>申请时间:</view>
...
<!--      <view class="item acea-row row-between" v-if="orderInfo.payPostage > 0">
    ...
    <view class="footer acea-row row-right row-middle" v-if="!refundOrder && offlineStatus">
      <template v-if="status.type == 0">
        ...
        <view class="bnt bg-color-red" @click="toPay('weixin')">立即付款</view>
      </template>
...
export default {
  ...
  methods: {
	  ...
	  changeAddress(addressInfo) {
	    ...
	    // this.addressInfo = addressInfo;
		this.orderInfo.realName =addressInfo.realName;
		this.orderInfo.userPhone =addressInfo.phone;
		this.orderInfo.userAddress =addressInfo.province+addressInfo.city+addressInfo.district+addressInfo.detail;
	  },
    copyClipboard,
    ...
    getDetail() {
      ...
      orderDetail(id)
        .then(res => {
		  ...
		  console.log(this.orderInfo.realName)
          this.getOrderStatus();
    ...
    async toPay(type) {
	  ...
	  var arr ={
        form:that.from,
        realName:that.orderInfo.realName,
        userPhone:that.orderInfo.userPhone,
        userAddress:that.orderInfo.userAddress,
	  };	  
      console.log(type, "支付方式");
      await payOrderHandle(this.orderInfo.orderId, type,JSON.stringify(arr));
      that.getDetail();
    }
  }
};
