	...
	<view class="apply-return">
		<view class="goodsStyle acea-row row-between" v-for="cart in orderInfo.cartInfo" :key="cart.id">
            ...
            : cart.productInfo.price
				...
				<!-- 实付款 -->
				<view class="text3  acea-row">
					<view>实付款</view>
					<view class="color"><text>￥</text>{{cart.productInfo.price*cart.cartNum}}+{{(cart.productInfo.otPrice-cart.productInfo.price)*cart.cartNum}}<span>(补贴券)</span></view>
				</view>				
			</view>
		...
		<view class="list">
			...
			<picker :value="reason" :range="reasonList" @change="changeReason">
				<view class="item acea-row row-between-wrapper">
					<view>退款原因:</view>
					<view class="nums">{{reason}}</view>
					<text class="iconfont icon-jiantou"></text>
				</view>
			</picker>
			<view class="item acea-row row-between-wrapper">
				<view>退款金额:</view>
				<view class="num"><text>￥</text>{{orderInfo.payPrice}}+{{orderInfo.sumDiscount}}<span>(补贴券)</span></view>
			</view>
		...
		<view class="returnBnt" @click="submit">提交申请</view>
	</view>
</template>
