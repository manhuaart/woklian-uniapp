<template>
  <view class="goodList">
    <view
      ...
      v-for="(item, goodlistIndex) in hotList"
      :key="goodlistIndex"
    >
      <view class="pictrue">
		...
		<image src="../../../static/imgs/pin.png" v-if="item.isPink==1" class="imgMiao" mode=""></image>
      </view>
      <view class="underline">
        <view class="text">
          <view class="line1">{{ item.storeName }}</view>
          <span class="linebox"><img src="../static/imgs/jin.png" class="imgw" />可用补贴券{{ item.otPrice - item.price }}</span>
